...
4. 関数名を入力: `generateShiftCurrentMonth`（Requestsシートから自動検出）
5. 「OK」をクリック
...
3. **Requestsシートに希望休を入力**（Date列に日付、各従業員列に`OFF`など）
4. メニューから「シフト生成」→「シフトを生成（Requestsシートから自動検出）」を選択
   - または、図形ボタンをタップ
5. 確認ダイアログが表示されるので、「OK」をクリック
6. シフト生成が完了すると、完了メッセージが表示されます（検出された月も表示されます）
7. `Schedule`シートと`Summary`シートを確認

**重要**: 
- Requestsシートに記載されている月のシフトが自動生成されます
- 月の指定は不要です。RequestsシートのDate列から自動的に検出されます
- 複数の月が含まれている場合は、最も多く出現する月が選択されます
...
1. **Requestsシートに希望休を入力**（Date列に日付、各従業員列に`OFF`など）
2. **スプレッドシートを開く**
3. **メニューから「シフト生成」→「シフトを生成（Requestsシートから自動検出）」を選択**
   - または、図形ボタンをタップ
4. **確認ダイアログで「OK」をクリック**
5. **「シフト生成中...」と表示される**
6. **完了メッセージが表示される**（検出された月も表示されます）
7. **`Schedule`シートと`Summary`シートを確認**
